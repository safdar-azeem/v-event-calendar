...
const doEventsOverlap = (eventA: CalendarEvent, eventB: CalendarEvent): boolean => {
   ...
   const endB = new Date(eventB.end).getTime() // Events overlap if one starts before the other ends, and vice-versa
...
export function calculateTimedEventLayout(eventsForDay: CalendarEvent[]): Map<string, TimedEventLayout> {
   ...
   if (!eventsForDay || eventsForDay.length === 0) {
      return layoutMap
   } // Sort events by start time, then by duration (longer first for consistent ordering)
   ...
   const sortedEvents = [...eventsForDay].sort((a, b) => {
      ...
      return endB - endA
   }) // Temporarily attach layout info to events for calculation

   const eventsWithLayout = sortedEvents.map((event) => ({
      event,
      overlaps: [] as any[],
      column: -1,
   })) // Find all overlapping events for each event
   ...
   } // Assign columns to avoid collisions
